...
public class Photoreceptor extends Neuron {
	...
	public Photoreceptor(int index, String type) {
		...
		//if there is a non-existing type of receptor given then runtimeexception
		if (type != "green" && type !="red" && type !="blue") {
			throw new RuntimeException(" Unfortunetlly there is a non-existing type of receptor given.");

		}
			this.type = type; // making the constructor for index and type
	...
	public void addSynapse(Synapse synapse) {
		// TODO
		if(synapse == null) {
			throw new RuntimeException("We can not have a null synapse..");
		}


		if(outgoingsynapses.size() > 0)  {  //runtime exception if there is more than one synapse
				throw new RuntimeException("Unfortunetlly there is more than one outgoing synapse.");
		}
		super.addSynapse(synapse); //adding a synapse
	}
